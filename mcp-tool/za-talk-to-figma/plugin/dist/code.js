...
(function() {
  ...
  const artifactPageName = (metadata) => {
    ...
    return `DualMind · ${spec.slice(0, 48)} · v${String((_b = metadata.specVersion) != null ? _b : "1")}`;
  };
  const artifactPageStem = (name) => name.replace(/\s*·\s*v[^·]+\s*$/i, "").trim().toLocaleLowerCase();
  const isPageLimitError = (error) => {
    const message = error instanceof Error ? error.message : String(error);
    return /starter plan/i.test(message) && /(?:3 pages|unlimited pages|page limit)/i.test(message);
  };
  const managedArtifactPageAtCapacity = (metadata, sourcePageId) => __async(null, null, function* () {
    ...
    if (!expectedStem.startsWith("dualmind · ") && !expectedStem.startsWith("zspector · ") && !expectedStem.startsWith("pm · ")) return null;
    const candidates = [];
    for (const page of figma.root.children) {
  ...
  const nextArtifactRootX = (page) => page.children.reduce((right, node) => {
  ...
  const applyArtifact = (params, requestId) => __async(null, null, function* () {
    ...
    if (!outputPage) {
      try {
        ...
        outputPage = yield managedArtifactPageAtCapacity(metadata, sourcePage.id);
        if (!outputPage) {
          throw new Error(
            `FIGMA_PAGE_LIMIT: ${error instanceof Error ? error.message : String(error)}. No same-product agent-managed Page can be reused safely.`
          );
        }
  ...
  const isProbablyInteractiveInstance = (node) => {
    const name = normalize(String(node.name || ""));
    const reactions = Array.isArray(node.reactions) ? node.reactions : [];
    return reactions.length > 0 || /button|input|field|checkbox|radio|switch|tab|list|item|picker|select|otp|cta|search|chip|cell/.test(name);
  };
  const isDecorativeInstance = (node) => {
    const name = normalize(String(node.name || ""));
  ...
  const handleProductCraftAuditRequest = (request) => __async(null, null, function* () {
    ...
    const componentDriftNodeIds = /* @__PURE__ */ new Set();
    const componentOverlapPairs = /* @__PURE__ */ new Set();
    const undersizedTouchTargetNodeIds = /* @__PURE__ */ new Set();
    const visibleTopLevelInstances = [];
    const visit = (node) => {
      var _a;
      visitedNodes += 1;
