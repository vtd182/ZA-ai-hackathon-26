...
func ValidateRPC(tool string, nodeIDs []string, params map[string]interface{}) string {
	...
	case "get_node":
		...
		if msg := validateReadTraversalParams(params); msg != "" {
			return msg
		}
	...
	case "get_design_context":
		...
		if detail, ok := params["detail"].(string); ok && detail != "" {
			switch detail {
			case "minimal", "summary", "compact", "full":
			default:
				return fmt.Sprintf("detail must be minimal, summary, compact, or full, got: %s", detail)
			}
		}
		if msg := validateReadTraversalParams(params); msg != "" {
			return msg
	...
	case "search_nodes":
		...
		if maxVisited, ok := params["maxVisited"].(float64); ok && maxVisited <= 0 {
			return "maxVisited must be a positive number"
		}
		if maxTimeMs, ok := params["maxTimeMs"].(float64); ok && maxTimeMs <= 0 {
			return "maxTimeMs must be a positive number"
		}
	...
	case "instantiate_component":
		...
		componentSetID, _ := params["componentSetId"].(string)
		if componentID == "" && componentSetID == "" {
			return "componentId or componentSetId is required"
		}
		...
		if componentSetID != "" && !ValidNodeID(componentSetID) {
			return fmt.Sprintf("componentSetId must use colon format e.g. 4029:12345, got: %s", componentSetID)
		}
		if pid, ok := params["parentId"].(string); ok && pid != "" && !ValidNodeID(pid) {
			return fmt.Sprintf("parentId must use colon format e.g. 4029:12345, got: %s", pid)
		}
		if raw, ok := params["variantProperties"]; ok {
			if _, ok := raw.(map[string]interface{}); !ok {
				return "variantProperties must be an object when provided"
			}
		}
...
func validateReadTraversalParams(params map[string]interface{}) string {
	if params == nil {
		return ""
	}
	...
	if maxNodes, ok := params["maxNodes"].(float64); ok && maxNodes <= 0 {
		return "maxNodes must be a positive number"
	}
	if maxTimeMs, ok := params["maxTimeMs"].(float64); ok && maxTimeMs <= 0 {
		return "maxTimeMs must be a positive number"
	}
