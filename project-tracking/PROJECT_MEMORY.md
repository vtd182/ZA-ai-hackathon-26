...
- **Current slice update (2026-07-30 ProductSpec surfaces):** ProductSpec no longer needs to lie about web/admin products as Mini Apps. `productType` now accepts `web_app`, `admin_dashboard`, `landing_page`, `desktop_tool` and `adaptive`; `screen.designSystemRoles` defaults to `[]`, so free/no-ZDS screens can be modeled without fake component roles. Clear admin dashboard synthesis now emits `productType: admin_dashboard` and zero DS roles, while Mini App synthesis keeps role hints. Figma free plans accept zero-slot primitive screens. Verification: `pnpm exec vitest run packages/domain/src/product-spec.test.ts packages/domain/src/artifact-brief.test.ts packages/agent-core/src/product-spec-synthesis.test.ts packages/connectors/src/figma-artifact-plan.test.ts`, `./run.sh typecheck`, `./run.sh test`, `./run.sh build`, and `git diff --check` pass.
- **Current slice update (2026-07-30 artifact approval visibility):** The renderer now shows the Figma `ArtifactBrief` in the approval panel before any external write: mode, surface, output policy, DS policy, verification count and ProductSpec hash. Execution stage labels now name the governed lifecycle (`ArtifactBrief + plan`, `Guard preflight`, `Write có approval`, `Read-back`, `Verify read-back`) so demo users can see what Agent Core is doing rather than only seeing a blackbox "AI/design" status. Verification: `./run.sh typecheck`, `./run.sh test`, `./run.sh build`, and `git diff --check` pass.
- **Current slice update (2026-07-30 error classification):** Renderer error toasts now classify failures by contract (`ProductSpec`, `ArtifactBrief`, `Figma MCP`, `Figma craft`, `Read-back verification`, `Provider`, `Canvas`, `Runtime`) and include a next action. This keeps demo failures product-readable instead of raw stack-like strings. Verification: `pnpm exec vitest run apps/desktop/src/renderer/src/error-classifier.test.ts`, `./run.sh typecheck`, `./run.sh test`, `./run.sh build`, and `git diff --check` pass.
- **Current slice update (2026-07-30 demo rehearsal):** AgentRouter now advertises `remoteResume: true`, matching the app-managed Codex bridge that persists remote refs outside the user's personal `~/.codex`. Figma can use AgentRouter for creative blueprint/craft, but Agent Core still owns immutable ArtifactBrief, approval, Figma MCP write and read-back verification. Added `project-tracking/DEMO_REHEARSAL.md` plus `./run.sh demo-rehearsal`, which verifies clear admin brief -> approved `admin_dashboard` ProductSpec -> no-ZDS `free_adaptive` ArtifactBrief -> selected-page Figma plan with zero component roles and no missing-role preflight issue. Verification: `./run.sh demo-rehearsal`, `./run.sh typecheck`, `./run.sh test`, `./run.sh build`, and `git diff --check` pass.
- **Current slice update (2026-07-30 provider transparency):** Renderer provider settings now show a visible runtime contract for each provider. AgentRouter copy says it can run reasoning + Figma craft through the managed Codex Responses bridge, while Figma/Jira/Zdoc writes still require Agent Core approval, MCP execution and read-back. This makes the governance value visible in the app instead of only in docs. Verification: `pnpm exec vitest run apps/desktop/src/renderer/src/provider-runtime-copy.test.ts`, `./run.sh typecheck`, `./run.sh test`, `./run.sh build`, and `git diff --check` pass.
- **Current slice update (2026-07-30 package-wide ArtifactBrief):** ArtifactBrief is now shared across the kickoff package. Mock Jira/backlog and Mock Zdoc/PRD plans include typed briefs created from the same ProductSpec payload hash as Figma. Their snapshots preserve the brief and verification fails on `ARTIFACT_BRIEF_MISMATCH`, making cross-artifact drift detectable instead of trusting connector success. Verification: `pnpm exec vitest run packages/connectors/src/mock-connectors.test.ts packages/domain/src/artifact-brief.test.ts`, `./run.sh typecheck`, `./run.sh test`, `./run.sh build`, and `git diff --check` pass.
- **Current slice update (2026-07-30 approval contract visibility):** Artifact approval UI now shows package-wide `Kickoff contracts` for every prepared target with an ArtifactBrief: Figma, Backlog mock and PRD.md. Each card exposes target, mode, surface, output policy, DS policy, verification count and ProductSpec hash before the user approves writes. Verification: `pnpm exec vitest run apps/desktop/src/renderer/src/artifact-brief-copy.test.ts`, `./run.sh typecheck`, `./run.sh test`, `./run.sh build`, and `git diff --check` pass.
- **Current slice update (2026-07-30 review bundle contracts):** Thread export now writes `artifact-contracts.md` and `artifact-contracts.json`, and embeds the same contract summary in `review-bundle.json`. The summary records per-target ArtifactBrief, payload hash, plan hash, execution receipt, verification state and whether every prepared artifact shares one ProductSpec source hash. Verification: `pnpm exec vitest run apps/desktop/src/main/export-bundle.test.ts`, `./run.sh typecheck`, `./run.sh test`, `./run.sh build`, and `git diff --check` pass.
- **Current slice update (2026-07-30 ProductSpec readiness UI):** Renderer now computes a shared ProductSpec readiness contract in `productspec-readiness.ts`. Delivery, ProductSpec overview and compact inspector expose Draft/Confirmed truth, product surface, artifact readiness, traceability blockers and next actions; kickoff package preparation is gated on approved, traced ProductSpec readiness. The visible copy clarifies that AgentRouter/Codex can create Figma blueprint/craft, but Agent Core still owns approval, Figma MCP write and read-back verification. Verification: `pnpm exec vitest run apps/desktop/src/renderer/src/productspec-readiness.test.ts`, `./run.sh typecheck`, `./run.sh test`, `./run.sh build`, and `git diff --check` pass.
- **Current slice update (2026-07-30 Canvas collaboration UI):** Chat now has a `Canvas collaboration loop` panel whenever the thread has canvas content. It keeps Vietnamese selection labels intact, offers focused ask/refine drafts, triggers the same CanvasWorkspace sync path from chat, and exposes guarded canvas promotion only while ProductSpec is still unconfirmed or empty. Smoke selector drift was updated from `.selection-chip` to the new panel. Verification: `pnpm exec vitest run apps/desktop/src/renderer/src/canvas-collaboration-copy.test.ts packages/persistence/src/history-store.test.ts`, `./run.sh typecheck`, `./run.sh test`, `./run.sh build`, and `git diff --check` pass. Note: full `./run.sh smoke` was attempted but is not used as evidence here because the broad seeded change/artifact path did not complete in the current local state.
- **Current slice update (2026-07-30 AgentRouter/Figma sharpness):** AgentRouter/Figma boundary is now clearer in both implementation and demo evidence: AgentRouter-style route output can stage change previews from structured intent even when commands are cleared, the meal-ordering demo starts from an approved Delivery baseline, and approved impact keeps ProductSpec v2 confirmed before executing verified Figma/Jira/Zdoc artifacts. The Figma setup smoke selector now targets the actual Figma button instead of the AI Canvas chip. Verification: `pnpm exec vitest run fixtures/meal-ordering/src/index.test.ts packages/agent-core/src/impact.test.ts packages/persistence/src/history-store.test.ts packages/reasoning/src/index.test.ts`, `./run.sh typecheck`, `./run.sh test`, `./run.sh build`, `PM_AGENT_SMOKE_RESET_COUNT=1 ./run.sh smoke`, and `git diff --check` pass.
- **Current slice update (2026-07-30 Workflow state receipt):** Chat now renders a canonical `Workflow state receipt` from app state, not provider prose. It tells the user whether the thread is conversation/canvas draft, Draft ProductSpec, Confirmed ProductSpec, pending immutable kickoff plan, impact preview, partial artifact failure or read-back verified kickoff. A screenshot review of the reset smoke shows `Kickoff verified` visible above the transcript without covering composer/canvas. Verification: `pnpm exec vitest run apps/desktop/src/renderer/src/workflow-state-receipt.test.ts`, `./run.sh typecheck`, `./run.sh test`, `./run.sh build`, `PM_AGENT_SMOKE_RESET_COUNT=1 ./run.sh smoke`, visual review of `/var/folders/0s/jn8plpxx1hq0d03tywsm69640000gn/T/pm-agent-smoke.png`, and `git diff --check` pass.
- **Current slice update (2026-07-30 Slash change controls):** Added `/change remove [target]` and `/scope drop ...` as deterministic, app-owned change-impact controls. The command resolves target requirements through Agent Core, stages before/after or ambiguity, switches to Change view and never calls a provider or writes artifacts before approval. `/help` now frames Figma create around approval/write/read-back. Verification: `pnpm exec vitest run apps/desktop/src/main/slash-commands.test.ts apps/desktop/src/renderer/src/workflow-state-receipt.test.ts`, `./run.sh typecheck`, `./run.sh test`, `./run.sh build`, `PM_AGENT_SMOKE_RESET_COUNT=1 ./run.sh smoke`, and `git diff --check` pass.
- **Current slice update (2026-07-30 BGK install handoff):** Added [docs/JUDGE_INSTALL_GUIDE.md](../docs/JUDGE_INSTALL_GUIDE.md) and linked it from README. The guide explains GitHub Release asset selection, macOS Gatekeeper bypass, Windows SmartScreen, Figma Desktop plugin import, ZDS/no-ZDS target selection and common troubleshooting, including that `latest.yml`/`*.blockmap` are not manual installers. Verification: `git diff --check` passes.
...
### BUG-060 - Stale canvas save after demo reset violated the thread foreign key

- **Status:** FIXED
- **Found:** 2026-07-30, smoke reset while improving canvas collaboration UI.
- **Symptom:** Electron logged `Error occurred in handler for 'canvas:save': SqliteError: FOREIGN KEY constraint failed` after demo reset.
- **Trigger:** Reset demo workspace while the mounted canvas still had a delayed/unmount save for a thread deleted by reset.
- **Root cause:** `HistoryStore.saveCanvas` updated `conversation_threads` but did not check whether the thread still existed before inserting into `canvas_documents`, whose `thread_id` references `conversation_threads(id)`.
- **Fix:** `saveCanvas` now returns from its transaction when the thread update touches zero rows, making stale renderer saves after reset a no-op.
- **Regression test:** `packages/persistence/src/history-store.test.ts` covers saving a stale canvas after `resetDemoWorkspace`.
- **Caveat:** This intentionally ignores only missing-thread saves; existing-thread canvas persistence and checkpoints still write normally.

### BUG-061 - Route-mode change intent was ignored after commands were cleared

- **Status:** FIXED
- **Found:** 2026-07-30, AgentRouter/Figma sharpness smoke.
- **Symptom:** The seeded demo prompt `Bỏ payment khỏi MVP` produced a normal assistant reply instead of a change preview; ProductSpec stayed v1 and artifacts were not prepared/executed.
- **Trigger:** Route-mode provider response with `intent.kind === 'change'` and `commands: []`, matching the provider-safe contract where chat routing does not carry canvas/UI mutation commands.
- **Root cause:** `chat:send` still staged removal changes only when it found a legacy `remove_card` command. Earlier product hardening intentionally cleared commands in route mode, so the structured change intent was silently skipped. The demo seed also advertised a Delivery runtime while the persisted thread phase was `discover` and the fixture ProductSpec was still `draft`.
- **Fix:** `chat:send` now creates an app-owned `changeRequest` from the structured `change` intent, using legacy `remove_card` only as a fallback query. The meal-ordering fixture is an approved ProductSpec and the seeded demo thread persists as `deliver`. Impact output preserves approved status only when the source spec is already approved.
- **Regression test:** Targeted fixture/impact/history/reasoning tests cover approved demo baseline, approved-vs-draft impact status, seeded `deliver` phase and commands-cleared structured change intent. `PM_AGENT_SMOKE_RESET_COUNT=1 ./run.sh smoke` now verifies ProductSpec v2, payment removed and Figma/Jira/Zdoc read-back verified.
- **Caveat:** This does not let providers mutate canvas or Figma directly; providers propose intent/craft while Agent Core resolves, approves, writes through MCP/connectors and verifies read-back.

### BUG-062 - Creative Figma slots inferred optional undefined content

- **Status:** FIXED
- **Found:** 2026-07-30, typecheck after adding workflow state receipt.
- **Symptom:** `./run.sh typecheck` failed in `packages/connectors/src/figma-artifact-plan.ts` because creative component slots inferred `content` as `{ text: string } | { text?: undefined }`, which is not assignable to the `DesignSlot` contract `Record<string, string>`.
- **Trigger:** A creative blueprint component with no `componentText`.
- **Root cause:** The inline `.map()` callback lacked an explicit `DesignSlot` return type, so TypeScript inferred the empty content object as an optional-text union instead of the intended record.
- **Fix:** Annotated the creative slot mapper as `DesignSlot`, making `content: {}` type-check as the explicit slot contract while preserving text when present.
- **Regression test:** `./run.sh typecheck` now passes; full `./run.sh test` and `./run.sh build` also pass.
- **Caveat:** This is a typing/contract fix only; it does not change Figma layout behavior.
