...
- **Evidence:** conversation routing now uses a compact collaboration schema with optional content-specific suggestions; IDEA_INTAKE threads render as Studio instead of leaking a lifecycle phase, selection is context rather than implicit edit permission, and `/studio explore|critique|sketch|refine` gives explicit control over conversation versus canvas writes. A live Electron replay kept an ordinary idea on a blank canvas, surfaced its privacy tension, then explicitly created a 13-node consent-first care flow and five distinct product screens. Selecting one privacy block, Syncing it into chat and accepting the suggested refinement changed exactly one shape with read-back while ProductSpec remained unchanged. Approved live Figma work starts from a sparse guarded scaffold, runs a bounded Codex MCP craft worker with design plus independent critic skills, requires observed screenshot/write/audit evidence and repairs the same Page from independent QA findings. The MCP exposes `apply_craft_patch` for alias-linked operations and `audit_product_craft` for screen/ZDS/prototype/copy/contrast/clipping gates. The app-owned live run recovered from a reconnected plugin and Figma Free's three-Page cap without deleting prior versions, then completed scaffold, screenshot, multiple refinement batches, two craft audits, final text scan and read-back in 396 seconds. Latest action root `512:19179` verified beside preserved `v2`; Jira and PRD remained at attempt 1. Visual evidence: `mcp-tool/za-talk-to-figma/artifacts/meal-ordering-app-worker-verified.png` and `zalo-login-craft-audited.png`. Plugin has 270 passing tests; skill validation, Go suite and full workspace release gates pass.

### `P0-FIG-010` Product-craft layout guard and skill import direction

- **Status:** DONE (2026-07-26)
- **Depends on:** P0-CRE-001
- **Deliver:** strengthen the independent Figma product-craft audit for component drift, screen containment, top-level ZDS overlap and touch-target fit; record the skill import/plugin direction inspired by tldraw Desktop Agent Skills.
...
- **Status:** DONE (2026-07-26)
- **Depends on:** P0-FIG-010
- **Deliver:** a global Skill Pack runtime that resolves bundled PM Lifecycle skills from dev source or packaged app resources, injects bounded skill content into provider workers, records pack identity/hash and includes packaging guidance/assets so production builds do not depend on the source repository layout.
